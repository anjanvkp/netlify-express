...
class EditExp extends Component {
    ...
    getExpById() {
        axios.get('/api/exp/edit/' + this.props.match.params.id, { headers: authHeader() })
            .then((response) => {
    ...
    handleSubmit = (event) => {
        ...
        axios.post('/api/exp/edit/' + this.props.match.params.id, {
            rid: rid,
            dt: dt,
            mode: mode,
            pto: pto,
